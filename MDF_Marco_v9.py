...
def vetorpoisson(k2,L,npontos,tc1,vc1,tc2,vc2):
    ...
    if k2 == 6:
        for i in range (1,npontos-1):
            ...
            vetor[i]=funcao
    if k2 == 7:
        for i  in range (1,npontos/2+1):
            vetor[i]=0
        for i in range (npontos/2+1,npontos-1):
            vetor[i]=1
    ...
    if tc1==0 and tc2==0:
        ...
        if k2==6:
            ...
            for i in range (npontos):
                x=i*h
                if x<0.5*L:
                    u=(x**3/(6.0*cte1))+((cte2/cte1)*(((((L**2)/24.0)*(1/cte2-1/cte1))+((2.0/L)*(vc2-vc1))-((L**2)/(3.0*cte2)))/(cte2/cte1+1)))*x+vc1
                if x>=0.5*L:
                    u=(x**3/(6.0*cte2))+(((((L**2)/24.0)*(1.0/cte2-1.0/cte1))+((2.0/L)*(vc2-vc1))-((L**2)/(3.0*cte2)))/(cte2/cte1+1))*x+vc2-((L**3)/(6.0*cte2))-(((((L**2)/24.0)*(1.0/cte2-1.0/cte1))+((2.0/L)*(vc2-vc1))-((L**2)/(3.0*cte2)))/(cte2/cte1+1.0))*L
                exata[i]=u
        if k2==7:
            h=1.0*L/(npontos-1)
            exata=[0]*npontos
            for i in range (npontos):
                x=i*h
                if x<0.5*L:
                    u=((L/2.0)+(5.0*L/8.0)+((vc2-vc1)/L))*x+vc1
                if x>=0.5*L:
                    u=(x**2)/2.0+((5.0*L/8.0)+((vc2-vc1)/L))*x+(L**2/8.0)+vc1
                exata[i]=u
...
if k1==2:
    k2=input('Escolha uma das funçoes da lista, para o caso homogeneo (fazer L=1):\n'
             '(1) => f(x) = x(x-1)\n'
             '(2) => f(x) = x\n'
             '(3) => f(x) = x(x-1)(x-0,5)\n'
             '(4) => f(x) = e**x\n'
             '(5) => f(x) = senx + cosx\n'
             '\n'
             'Para o caso heterogeneo, seguem as opcoes:\n'
             '(6)k1!=k2, f(x)=x, transicao em L/2\n'
             '(7)k1=k2, f1(x)=0, f2(x)=1, transicao em L/2\n')
