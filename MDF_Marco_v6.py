...
def avaliaerro(erroanterior,erroatual,npontos,refinamento,parada):
    difrelativa = 0
    diferro = erroatual - erroanterior
    if diferro<0:
        diferro = (-1.0)*diferro
    difrelativa = diferro/erroatual
    if difrelativa <= erroadm:
        parada = 1
    refinamento = refinamento + 1
    return npontos, difrelativa, refinamento, parada, diferro
...
while parada == 0:
    ...
    erroanterior, erroatual, solnumanterior = calculaerro(solnumerica,solnumanterior,npontos,refinamento,erroatual)

    npontos, difrelativa, refinamento, parada, diferro = avaliaerro(erroanterior,erroatual,npontos,refinamento,parada)
